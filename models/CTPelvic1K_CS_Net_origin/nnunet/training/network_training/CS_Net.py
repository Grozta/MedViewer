...
class cs_net(nn.Module):
    def __init__(self, in_channels, out_channels, num_filters):
        ...
        # TODO cs_net in_channels
        self.first = Conv_down2(in_channels, 23, True)
    ...
    def forward(self, x):
        # Slice aware encoder start
        ori_1 = self.avg(x[:, 1, :, :].unsqueeze(1))  # B*1*256*256
        ori_2 = self.avg(x[:, 2, :, :].unsqueeze(1))  # B*1*256*256
        ori_3 = self.avg(x[:, 3, :, :].unsqueeze(1))  # B*1*256*256
        x_1, x_2, x_3 = self.first(x)  # B*69*256*256
        conv0_1, conv1_1 = self.Conv_down1_1(torch.cat((x_1, ori_1), dim=1))  # B*70*256*256
        conv0_2, conv1_2 = self.Conv_down1_2(torch.cat((x_2, ori_2), dim=1))  # B*70*256*256
        conv0_3, conv1_3 = self.Conv_down1_3(torch.cat((x_3, ori_3), dim=1))  # B*70*256*256
        x = torch.cat((conv1_1, conv1_2, conv1_3), dim=1)  # B*210*256*256
        # Slice aware encoder end

        # start of net
        # B*210*256*256
        x = self.down(x)  # B*140*128*128
        x, conv2 = self.Conv_down2(x)  # B*280*64*64, 1*140*128*128
        x, conv3 = self.Conv_down3(x)  # B*560*32*32, B*280*64*64
        _, x = self.Conv_down4(x)  # B*560*32*32
        x = self.Conv_up1_2(x, conv3)  # B*280*64*64
        x = self.Conv_up1_3(x, conv2)  # B*140*128*128
        # end of net

        # Slice Attentive decoder
        x1 = self.se1(x)  # B*140*128*128
        x2 = self.se2(x)  # B*140*128*128
        x3 = self.se3(x)  # B*140*128*128

        x1_1 = self.Conv_up1_4(x1, conv0_1)  # B*70*256*256
        x2_1 = self.Conv_up2_4(x2, conv0_2)  # B*70*256*256
        x2_2 = self.Conv_up2_4_2(x2, conv0_2)  # B*70*256*256
        x3_1 = self.Conv_up3_4(x3, conv0_3)  # B*70*256*256

        x1_1 = self.up1(x1_1)  # B*70*512*512
        # x1_2=self.up1(x1_2)
        x2_1 = self.up2(x2_1)  # B*70*512*512
        x2_2 = self.up2(x2_2)  # B*70*512*512
        x3_1 = self.up3(x3_1)  # B*70*512*512
        # x3_2 = self.up3(x3_2)

        x1_1 = self.Conv_out1(x1_1)  # 1*4*512*512
        x2_1 = self.Conv_out2(x2_1 + x2_2)  # 1*4*512*512
        x3_1 = self.Conv_out3(x3_1)  # 1*4*512*512

        # x1_2 = self.Conv_out1_2(x1_2)
        x2_2 = self.Conv_out2_2(x2_2)  # 1*1*512*512
