import torch
import numpy as np
from models.CTPelvic1K_CS_Net_origin.nnunet.training.loss_functions.TopK_loss import TopKLoss
from models.CTPelvic1K_CS_Net_origin.nnunet.utilities.nd_softmax import softmax_helper
from models.CTPelvic1K_CS_Net_origin.nnunet.training.loss_functions.ND_Crossentropy import CrossentropyND, CrossentropyND_DeepS
from models.CTPelvic1K_CS_Net_origin.nnunet.utilities.tensor_utilities import sum_tensor
...
class GDiceLoss(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        # copy from https://github.com/LIVIAETS/surface-loss/blob/108bd9892adca476e6cdf424124bc6268707498e/losses.py#L29
        w: torch.Tensor = 1 / (einsum("bcxy->bc", y_onehot).type(torch.float32) + 1e-10)
...
class GDiceLossV2(nn.Module):
    ...
    def forward(self, net_output, gt):
        ...
        class_weights = Variable(1. / (torch.sqrt(target_sum)).clamp(min=self.smooth), requires_grad=False)
...
class DC_and_CE_Exclusion_loss_DeepS(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, aggregate="sum", ex=True, rate=2, deepspool=None):
        ...
        # self.ex = Exclusion_loss(self.dc)
        self.gdl = GDL(apply_nonlin=softmax_helper)
        # self.gdl = GDiceLossV2(apply_nonlin=softmax_helper)
        # self.gdc = GDiceLoss(apply_nonlin=softmax_helper)
        self.ex_choice = ex
        self.rate = rate
        assert self.rate >= 0
    def forward(self, net_outputs, target, sdf_heatmap):
        ...
        # targets = targets
        # gd_loss = self.gdl(net_outputs[0], targets[0])
