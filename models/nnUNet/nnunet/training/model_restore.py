...
def recursive_find_python_class(folder, trainer_name, current_module):
    tr = None
    print('recursive_find_python_class', folder)
    print('current_module: {}'.format(current_module))
    print('trainer_name: {}'.format(trainer_name))
    for importer, modname, ispkg in pkgutil.iter_modules(folder):
        print(modname, ispkg)
        if not ispkg:
            try:
                module_name = modname if current_module == '' else current_module + "." + modname
                ...
                print(e)
                continue
            print('importlib.import_module: {}'.format(m))
    ...
    if tr is None:
        for importer, modname, ispkg in pkgutil.iter_modules(folder):
            if ispkg:
                ...
                next_current_module = modname if current_module == '' else current_module + "." + modname
...
def restore_model(pkl_file, checkpoint=None, train=False, fp16=None):
    ...
    search_in = join(base_dir, 'models', 'nnUNet', 'nnunet', "training", "network_training")
    # FIXME 改成直接import
    tr = nnUNetTrainerV2
    # tr = recursive_find_python_class([search_in], name, current_module="models.nnUNet.nnunet.training.network_training")
