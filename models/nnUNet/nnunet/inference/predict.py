...
from models.nnUNet.nnunet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
from models.nnUNet.nnunet.postprocessing.connected_components import load_remove_save, load_postprocessing
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    shutil.copy(join(model, 'plans.pkl'), output_folder) # 将模型路径下的plans.pkl拷贝到output_folder

    assert isfile(join(model, "plans.pkl")), "Folder with saved model weights must contain a plans.pkl file"
    expected_num_modalities = load_pickle(join(model, "plans.pkl"))['num_modalities'] # 模态个数   1

    # check input folder integrity
    case_ids = check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities)  # 就是获取待预测文件的id 如 'RibFrac421'

    output_files = [join(output_folder, i + ".nii.gz") for i in case_ids] # 获取输出文件的地址
    all_files = subfiles(input_folder, suffix=".nii.gz", join=False, sort=True) # 获取待预测文件的文件名
